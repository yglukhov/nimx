...
method init(v: ControlsSampleView, w: Window, r: Rect) =
    procCall v.View.init(w, r)

    let label = newLabel(w, newRect(10, 10, 100, 20))
    let textField = newTextField(w, newRect(120, 10, v.bounds.width - 130, 20))
    textField.autoresizingMask = { afFlexibleWidth, afFlexibleMaxY }
    label.text = "Text field:"
    v.addSubview(label)
    v.addSubview(textField)

    let button = newButton(w, newRect(10, 40, 100, 22))
    button.title = "Button"
    button.onAction do():
        textField.text = "Click! "
    v.addSubview(button)

    let sc = SegmentedControl.new(w, newRect(120, 40, v.bounds.width - 130, 22))
    ...
    let checkbox = newCheckbox(w, newRect(10, 70, 50, 16))
    checkbox.title = "Checkbox"
    v.addSubview(checkbox)

    let progress = ProgressIndicator.new(w, newRect(120, 130, v.bounds.width - 130, 16))
    progress.autoresizingMask = { afFlexibleWidth, afFlexibleMaxY }
    v.addSubview(progress)

    let slider = Slider.new(w, newRect(120, 70, v.bounds.width - 130, 16))
    ...
    let vertSlider = Slider.new(w, newRect(v.bounds.width - 26, 150, 16, v.bounds.height - 160))
    vertSlider.autoresizingMask = { afFlexibleMinX, afFlexibleHeight }
    v.addSubview(vertSlider)

    let radiobox = newRadiobox(w, newRect(10, 90, 50, 16))
    radiobox.title = "Radiobox"
    v.addSubview(radiobox)

    let indeterminateCheckbox = newCheckbox(w, newRect(10, 130, 100, 16))
    ...
    let pb = PopupButton.new(w, newRect(120, 90, 120, 20))
    pb.items = @["Popup button", "Item 1", "Item 2"]
    v.addSubview(pb)

    sharedAssetManager().getAssetAtPath("cat.jpg") do(i: Image, err: string):
        discard newImageButton(v, w, newPoint(260, 90), newSize(32, 32), i)

    let tfLabel = newLabel(w, newRect(330, 150, 150, 20))
    tfLabel.text = "<-- Enter some text"
    let tf1 = newTextField(w, newRect(10, 150, 150, 20))
    let tf2 = newTextField(w, newRect(170, 150, 150, 20))
    tf1.onAction do():
    ...
    let cp = newColorPickerView(w, newRect(0, 0, 400, 170))
    ...
    sharedAssetManager().getAssetAtPath("tile.png") do(i: Image, err: string):
        let imageView = newImageView(w, newRect(0, 400, 300, 150), i)
        v.addSubview(imageView)

        let popupFillRule = newPopupButton(v, w, newPoint(420, 400), newSize(100, 20), ["NoFill", "Stretch", "Tile", "FitWidth", "FitHeight"])
