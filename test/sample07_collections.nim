...
method init(v: CollectionsSampleView, gfx: GraphicsContext, r: Rect) =
    procCall v.View.init(gfx, r)

    setTimeout 0.2, proc() =
        let collection = @["one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "eleven"]

        let collectionView = newCollectionView(gfx, newRect(0, 0, 450, 250), newSize(50, 50), LayoutDirection.LeftToRight)
        collectionView.numberOfItems = proc(): int =
            return collection.len()
        collectionView.viewForItem = proc(i: int): View =
            result = newView(gfx, newRect(0, 0, 100, 100))
            discard newLabel(result, gfx, newPoint(0, 0), newSize(50, 50), collection[i])
        ...
        discard newLabel(v, gfx, newPoint(470, 5), newSize(100, 10), "Layout direction:")

        let popupDirectionRule = newPopupButton(v, gfx, newPoint(470, 20), newSize(100, 20), ["LeftToRight", "TopDown"])
        popupDirectionRule.onAction do():
            collectionView.layoutDirection = popupDirectionRule.selectedIndex().LayoutDirection
            collectionView.updateLayout()

        discard newLabel(v, gfx, newPoint(470, 45), newSize(100, 10), "Layout width:")

        let popupLayoutWidth = newPopupButton(v, gfx, newPoint(470, 60), newSize(100, 20), ["Auto", "1", "2", "3", "4"])
        popupLayoutWidth.onAction do():
            collectionView.layoutWidth = popupLayoutWidth.selectedIndex()
            collectionView.updateLayout()

        discard newLabel(v, gfx, newPoint(470, 85), newSize(100, 10), "Item size:")

        let popupItemSize = newPopupButton(v, gfx, newPoint(470, 100), newSize(100, 20), ["50", "100", "150"])
        ...
        discard newLabel(v, gfx, newPoint(470, 125), newSize(100, 10), "Offset:")

        let offsetField = newTextField(gfx, newRect(newPoint(470, 140), newSize(100, 20)))
