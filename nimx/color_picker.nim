...
proc newColorComponentTextField(w: Window, r: Rect, comp: ColorComponent): ColorComponentTextField =
    result.new
    result.init(w, r)
...
proc newColorPickerH(w: Window, r: Rect): ColorPickerH =
    ## Hue picker constructor
    result.new
    result.init(w, r)

method init(cph: ColorPickerH, w: Window, r: Rect) =
    procCall cph.View.init(w, r)
...
proc newColorPickerS(w: Window, r: Rect): ColorPickerS =
    ## Saturation picker constructor
    result.new
    result.init(w, r)

method init(cps: ColorPickerS, w: Window, r: Rect) =
    procCall cps.View.init(w, r)
...
proc newColorPickerV(w: Window, r: Rect): ColorPickerV =
    ## Saturation picker constructor
    result.new
    result.init(w, r)

method init(cpv: ColorPickerV, w: Window, r: Rect) =
    procCall cpv.View.init(w, r)
...
proc newColorPickerCircle(w: Window, defaultPalette: ColorPickerPalette, radius: Coord, frame: Rect): ColorPickerCircle =
    result = ColorPickerCircle.new(w, frame)
...
proc newColorPickerView*(w: Window, r: Rect = newRect(0, 0, 300, 200), defaultPalette = ColorPickerPalette.HSV, backgroundColor: Color = newGrayColor(0.35, 0.8)): ColorPickerView =
...
proc newColorView*(w: Window, r: Rect, color: Color, main: bool = false): ColorView =
...
method init(cv: ColorView, w: Window, r: Rect) =
    procCall cv.View.init(w, r)
...
method init*(cpv: ColorPickerView, w: Window, r: Rect) =
    ...
    cpv.circle = newColorPickerCircle(w, ColorPickerPalette.HSV, circleRadius, circleRect)
    ...
    cpv.chosenColorView = newColorView(w, newRect(margin * 2.0 + 20, margin * 2.0, r.height / 4.0, r.height / 4.0), newGrayColor(1.0), main = true)
    ...
    for hitem in 0 ..< historySize:
        let newItem = newColorView(w, newRect(
    ...
    let hLabel = newLabel(cpv, w, newPoint(margin, coff + margin), newSize(20, 20), "H:")
    let sLabel = newLabel(cpv, w, newPoint(margin, coff + margin * 2 + 20), newSize(20, 20), "S:")
    let vLabel = newLabel(cpv, w, newPoint(margin, coff + margin * 3 + 40), newSize(20, 20), "V:")

    # H Component View
    cpv.tfH = newColorComponentTextField(w, newRect(margin + 20 + margin, coff + margin, 40, 20), ColorComponent.H)
    cpv.tfH.text = $cpv.currentColor.h
    cpv.addSubview(cpv.tfH)

    cpv.cpH = newColorPickerH(w, newRect(margin + 20 + 40 + margin + margin, coff + margin, r.width - rightSize - 40 - margin * 4.0 - 20, 20))
    cpv.addSubview(cpv.cpH)

    # S Component View
    cpv.tfS = newColorComponentTextField(w, newRect(margin + 20 + margin, coff + margin * 2 + 20, 40, 20), ColorComponent.S)
    cpv.tfS.text = $cpv.currentColor.s
    cpv.addSubview(cpv.tfS)

    cpv.cpS = newColorPickerS(w, newRect(margin + 20 + 40 + margin + margin, coff + margin * 2 + 20, r.width - rightSize - 40 - margin * 4.0 - 20, 20))
    cpv.addSubview(cpv.cpS)

    # V Component View
    cpv.tfV = newColorComponentTextField(w, newRect(margin + 20 + margin, coff + margin * 3 + 40, 40, 20), ColorComponent.V)
    cpv.tfV.text = $cpv.currentColor.v
    cpv.addSubview(cpv.tfV)

    cpv.cpV = newColorPickerV(w, newRect(margin + 20 + 40 + margin + margin, coff + margin * 3 + 40, r.width - rightSize - 40 - margin * 4.0 - 20, 20))
