...
proc newFormView*(w: Window, r: Rect, numberOfFields: int, adjustFrameHeight: bool = true): FormView =
    result.new()
    ...
    for i in 0 .. < numberOfFields:
        let label = newLabel(w, newRect(0, i.Coord * fieldHeight, r.width / 3, fieldHeight))
        result.addSubview(label)
        let value = newTextField(w, newRect(r.width / 3, i.Coord * fieldHeight, r.width / 3 * 2, fieldHeight))
...
proc newFormView*(w: Window, r: Rect, fieldNames: openarray[string], adjustFrameHeight: bool = true): FormView =
    result = newFormView(w, r, fieldNames.len, adjustFrameHeight)
