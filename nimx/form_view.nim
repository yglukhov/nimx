...
proc newFormView*(gfx: GraphicsContext, r: Rect, numberOfFields: int, adjustFrameHeight: bool = true): FormView =
    result.new()
    ...
    for i in 0 .. < numberOfFields:
        let label = newLabel(gfx, newRect(0, i.Coord * fieldHeight, r.width / 3, fieldHeight))
        result.addSubview(label)
        let value = newTextField(gfx, newRect(r.width / 3, i.Coord * fieldHeight, r.width / 3 * 2, fieldHeight))
...
proc newFormView*(gfx: GraphicsContext, r: Rect, fieldNames: openarray[string], adjustFrameHeight: bool = true): FormView =
    result = newFormView(gfx, r, fieldNames.len, adjustFrameHeight)
