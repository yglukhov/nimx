...
method init(mxy: PlotXY, gfx: GraphicsContext, r: Rect) =
  procCall mxy.Control.init(gfx, r)
...
proc newPlotXY*(gfx: GraphicsContext, r: Rect, model: ModelXYColor[float64]): PlotXY =
  result.new()
  result.model = model
  result.init(gfx, r)
...
method draw*(mxy: PlotXY, r: Rect) =
  ...
  template gfx: untyped = mxy.gfx

  ## Draw grid
  gfx.strokeColor = newGrayColor(0.7)
  gfx.strokeWidth = 1
  ...
  gfx.strokeColor = blackColor()
  gfx.strokeWidth = 2

  if mxy.model.len() > 0:
    if mxy.drawMedian:
      gfx.strokeColor = newColor(0.0, 1.0, 0.0)
      gfx.drawLine(newPoint(mxy.poly[0], mxy.poly[1]), newPoint(mxy.poly[mxy.poly.len() - 2], mxy.poly[mxy.poly.len() - 1]))
    ...
    for i in countup(0, mxy.poly.len()-3, 2):
      gfx.strokeColor = mxy.model[(i/2).int].color
      gfx.fillColor = gfx.strokeColor

      if mxy.highlightedPoint != -1:
        if i == mxy.highlightedPoint or i == mxy.highlightedPoint + 1:
          gfx.drawEllipseInRect(newRect(mxy.poly[i] - 6, mxy.poly[i+1] - 6, 12, 12))
      gfx.drawEllipseInRect(newRect(mxy.poly[i] - mxy.dotSize.Coord, mxy.poly[i+1] - mxy.dotSize.Coord, mxy.dotSize.Coord * 2, mxy.dotSize.Coord * 2))
    gfx.drawEllipseInRect(newRect(mxy.poly[^2] - mxy.dotSize.Coord, mxy.poly[^1] - mxy.dotSize.Coord, mxy.dotSize.Coord * 2, mxy.dotSize.Coord * 2))

  gfx.fillColor = blackColor()
  gfx.strokeColor = blackColor()
  let font = systemFont(gfx.fontCtx)

  ## Draw title
  var pt = centerInRect(font.sizeOfString(mxy.title), newRect(0.0, 0.0, r.size.width, mxy.boundary))
  gfx.drawText(font, pt, mxy.title)
  ...
  for i in 0..mxy.gridstep.int:
    ...
    gfx.drawText(font, pt, $stepValue.int)
