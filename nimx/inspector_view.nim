...
method init*(v: InspectorView, gfx: GraphicsContext, r: Rect) =
    procCall v.LinearLayout.init(gfx, r)
    v.horizontal = false
...
proc setInspectedObject*[T](v: InspectorView, o: T) =
    ...
    visitor.commit = proc() =
        v.addSubview(propertyEditorForProperty(v.gfx, oo, visitor.name, visitor.setterAndGetter, onChange = onChanged(visitor)))
